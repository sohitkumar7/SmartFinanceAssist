...
function AddTransactionForm() {
  ...
  // Get account from navigation state (when coming from AccountCard)
  const location = useLocation();
  const passedAccount = location.state?.account;
  
  // Use passed account if available, otherwise find default account
  const defaultAccount = passedAccount || allAccount?.find((acc) => acc.isDefault);
      ...
      if (res?.payload?.success) {
        ...
        if (form.type === "EXPENSE" && defaultAccount?._id) {
          ...
          dispatch(fetchBudget(defaultAccount._id)).then(() => {
            ...
            // Navigate after fetch completes with a small delay to ensure data is synced
            setTimeout(() => {
              setIsSubmitting(false);
              navigate("/dashboard");
            }, 500);
          ...
          dispatch(fetchallAccount());
          dispatch(DashfetchAllTransaction({ userId: backendUser._id }));
          // Navigate with a small delay to ensure data is synced
          setTimeout(() => {
            setIsSubmitting(false);
            navigate("/dashboard");
          }, 500);
    ...
    <div className="max-w-lg mx-auto p-5 bg-white rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className={`w-full text-white p-2 rounded flex items-center justify-center gap-2 ${
